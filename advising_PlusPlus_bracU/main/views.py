...
def examScheduleCse(course):
    ...
    with open(pwd+'/CentralExamDates.html', 'r') as f:
        html_txt = f.read()
...
def course_time_table(course, sec):
    pwd = os.path.dirname(__file__)
    with open(pwd+'/Tabular.html', 'r') as f:
        html_txt = f.read()
    soup = BeautifulSoup(html_txt, 'lxml')
...
def main(Course, Section):
    av_seat = available_seats(Course, Section)
    exdate = examScheduleCse(Course)
    c_time = course_time_table(Course, Section)

    return [av_seat, exdate, c_time]
...
def index(request):
    if request.method == 'POST':
        ...
        a, b, c = main(course, sec)
        return render(request, 'index2.html', {'a1': a[0], 'b': b, 'c': c, 'a2': a[1]})
